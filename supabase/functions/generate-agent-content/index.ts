...
serve(async (req) => {
  ...
  try {
    ...
    // Get business configuration using slug
...
// Helper functions
function getAgentConfig(businessSlug: string): BusinessAgentConfig | null {
  const configs: Record<string, BusinessAgentConfig> = {
    'fight-flow-academy': {
...
Generate content that inspires creators while educating them on how to build compelling AI characters. Make people excited about the creative possibilities.`
  ...
  return configs[businessSlug] || null;
}