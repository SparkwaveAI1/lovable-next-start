...
CREATE POLICY "Users can view sequences for their businesses" ON email_campaign_sequences
  FOR SELECT USING (
    business_id IN (SELECT business_id FROM user_business_access WHERE user_id = auth.uid())
  );
...
CREATE POLICY "Users can view sequence steps" ON email_campaign_sequence_steps
  FOR SELECT USING (
    sequence_id IN (
      SELECT id FROM email_campaign_sequences WHERE business_id IN (
...
CREATE POLICY "Users can view sequence sends" ON email_sequence_sends
  FOR SELECT USING (
    enrollment_id IN (
      SELECT id FROM email_sequence_enrollments WHERE business_id IN (
        SELECT business_id FROM user_business_access WHERE user_id = auth.uid()
      )
    )
  );
