...
export function MediaPicker({ open, onClose, onSelect, selectedId }: MediaPickerProps) {
  ...
  const fetchAssets = useCallback(async (searchQuery: string, pageNum: number, reset = false) => {
  ...
  const getAssetUrl = (asset: MediaAsset) => {
        ...
        <div className="flex-1 overflow-y-auto">
          {assets.length === 0 && !loading ? (
            <div className="flex flex-col items-center justify-center py-12 text-slate-500">
              ...
              <p className="text-sm">{debouncedQuery ? "No media found" : "Search for media files"}</p>
            </div>
          ) : (
            <div className="grid grid-cols-4 gap-3">
              {assets.map(asset => {
                const isSelected = asset.id === selectedId;
                const isImage = asset.file_type?.startsWith("image/") || ["jpg","jpeg","png","gif","webp","svg"].some(ext => asset.file_name.toLowerCase().endsWith(ext));
                return (
                  <button
                    key={asset.id}
                    ...
                    className={`relative rounded-lg overflow-hidden border-2 transition-all aspect-square bg-slate-100 hover:border-indigo-400 ${
                      isSelected ? "border-indigo-600 ring-2 ring-indigo-300" : "border-slate-200"
                    }`}
                  >
                    {isImage ? (
                      <img
